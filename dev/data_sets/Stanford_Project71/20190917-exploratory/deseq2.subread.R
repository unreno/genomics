...
for( ref in c('hairpin','mature') ) {

	pdf(paste0('deseq.',ref,'.subread.pdf'))
	
	df = read.csv(paste0('subread.',ref,'.featureCounts.csv'),
		comment.char="#",
		sep='\t')
	head(df)
	...
	colnames(df)<-sub(".subread.bam","",colnames(df))
	head(df)
	...
	for( id in rownames(res[1:12,])){
		print(id)
		plotCounts(dds, gene=id, intgroup=c('cc'))
	}
	...
	plotPCA( DESeqTransform( se ), intgroup=c( 'cc' ) )
